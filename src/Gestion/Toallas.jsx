...
const Toallas = () => {
    ...
    useEffect(() => {
        const q = query(collection(db, 'lavanderia', 'toallas', 'movimientos'), orderBy('timestamp', 'asc'));
    ...
    const handleGuardarEdicion = async () => {
        if (currentMovimiento && newCantidad !== '') {
            const movimientoRef = doc(db, 'lavanderia', 'toallas', 'movimientos', currentMovimiento.id);
    ...
    const confirmarBorrar = async () => {
        if (movimientoToDelete) {
            const movimientoRef = doc(db, 'lavanderia', 'toallas', 'movimientos', movimientoToDelete.id);
    ...
    return (
        <div className="overflow-x-hidden w-screen h-screen">
            ...
            <div className="flex justify-center items-center h-full">
                <div className="w-3/4">
                    {/* Encabezado agregado */}
                    <h2 className="text-2xl font-semibold mb-4">Movimientos de Toallas</h2>
