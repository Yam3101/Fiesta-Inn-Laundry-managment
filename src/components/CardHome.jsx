...
const CardHome = ({ nameCard }) => {
		...
		if (showGestion) {
			...
			const unsubscribe = onSnapshot(q, (snapshot) => {
				...
				snapshot.forEach((doc) => {
					const data = doc.data();
					const cantidad = Number.parseInt(data.cantidad) || 0; // Asegúrate de que cantidad sea un número
					movs.push({
						id: doc.id,
						cantidad: cantidad,
	...
	return (
		<div className="flex flex-wrap">
			<div
				className={`bg-white shadow-md shadow-stone-600 w-80 rounded-md text-center py-6 relative mb-6 ${
					showGestion ? "mr-2" : ""
				}`}
			>
